...
// Load products from catalog_seed.json if it exists, otherwise use fallback
...
if (fs.existsSync(seedPath)) {
    try {
        ...
        console.log(`Loaded ${products.length} products from catalog_seed.json`);
    } catch (error) {
        console.error("Error reading catalog_seed.json:", error);
    }
} else {
    // Fallback for Sprint 2
    products = [
...
exports.setProducts = (newProducts) => {
